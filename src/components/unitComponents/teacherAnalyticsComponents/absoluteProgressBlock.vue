<template>
  ...
  <div class="analytic-block--options">
    <p>Выберите группу:</p>
    <select @click="hideAbsoluteProgress()" class="select-group" v-model="groups">
      <option
    ...
    <p>Выберите тест:</p>
    <select @click="hideAbsoluteProgress()" class="select-grade" v-model="titleTest">
      <option
    ...
    <button @click="getResultProgress">{{visible ? 'Скрыть' : 'Показать'}}</button>
  </div>
  <transition name="fade">
    <p v-if="visible" class="text-analitics">Абсолютная успеваемость группы {{groups}} равна <span>{{result.toFixed(1)}} %</span></p>
  </transition>
</template>

<script>
export default {
  name: "absoluteProgressBlock",
  props: {
    dataGroup: Object,
    dataResult: Object
  },
  ...
  methods: {
    ...
    getResultProgress() {
      ...
      this.result = ((otl + hor + ud) * 100) / this.searchStudent().length
    },
    // Скрыть прогресс
    hideAbsoluteProgress() {

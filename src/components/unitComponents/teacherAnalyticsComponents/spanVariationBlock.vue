<template>
  ...
  <div class="analytic-block--options">
    <p>Выберите группу:</p>
    <select @click="hideVariationProgress()" class="select-group" v-model="groups" >
      <option
    ...
    <p>Выберите тест:</p>
    <select class="select-grade" @click="hideVariationProgress()" v-model="titleTest">
      <option
    ...
    <button @click="getResultVariation">{{visible ? 'Скрыть' : 'Показать'}}</button>
  </div>
  <transition name="fade">
   <p v-if="visible" class="text-analitics">Размах вариации для группы {{groups}} равен <span>{{result}}</span></p>
  </transition>
</template>

<script>
export default {
  name: "spanVariationBlock",
  props: {
    dataGroup: Object,
    dataResult: Object
  },
  ...
  methods: {
    ...
    getResultVariation () {
      ...
      let maxNumber = Math.max.apply(null, this.searchRating())
      let minNumber = Math.min.apply(null, this.searchRating())
      this.result = maxNumber - minNumber
      if (this.result === -Infinity) this.result = 0
    },
    hideVariationProgress () {

<template>
  ...
  <div class="analytic-block--options">
    <p>Выберите группу:</p>
    <select @click="hideDispersion()" class="select-group" v-model="groups" >
      <option
    ...
    <p>Выберите тест:</p>
    <select class="select-grade" @click="hideDispersion()" v-model="titleTest">
      <option
    ...
    <button @click="getDispersion">{{visible ? 'Скрыть' : 'Показать'}}</button>
  </div>
  <transition name="fade">
    <p v-if="visible" class="text-analitics">Дисперсия группы {{groups}} равна <span>{{result.toFixed(2)}}</span></p>
  </transition>
</template>

<script>
export default {
  name: "dispersionBlock",
  props: {
    dataGroup: Object,
    dataResult: Object
  },
